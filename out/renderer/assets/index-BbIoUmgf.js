...
const processBuffer = async (proxy, renderedBuffer, nativeOfflineAudioContext, options, outputChannelCount, processorConstructor, exposeCurrentFrameAndCurrentTime2) => {
  ...
  for (let i = 0; i < length; i += 128) {
    if (options.numberOfInputs > 0 && renderedBuffer !== null) {
      for (let j = 0; j < options.numberOfInputs; j += 1) {
        for (let k2 = 0; k2 < options.channelCount; k2 += 1) {
          copyFromChannel(renderedBuffer, inputs[j], k2, k2, i);
        }
      }
    }
    ...
    for (let j = 0; j < options.numberOfInputs; j += 1) {
      for (let k2 = 0; k2 < outputChannelCount[j]; k2 += 1) {
        if (outputs[j][k2].byteLength === 0) {
          outputs[j][k2] = new Float32Array(128);
        }
      }
    }
    try {
      ...
      if (processedBuffer !== null) {
        for (let j = 0, outputChannelSplitterNodeOutput = 0; j < options.numberOfOutputs; j += 1) {
          for (let k2 = 0; k2 < outputChannelCount[j]; k2 += 1) {
            copyToChannel(processedBuffer, outputs[j], k2, outputChannelSplitterNodeOutput + k2, i);
          }
    ...
    scriptProcessorNode.onaudioprocess = ({ inputBuffer, outputBuffer }) => {
      if (audioWorkletProcessor !== null) {
        ...
        for (let i = 0; i < bufferSize; i += 128) {
          for (let j = 0; j < options.numberOfInputs; j += 1) {
            for (let k2 = 0; k2 < options.channelCount; k2 += 1) {
              copyFromChannel(inputBuffer, inputs[j], k2, k2, i);
            }
          }
          if (processorConstructor.parameterDescriptors !== void 0) {
            processorConstructor.parameterDescriptors.forEach(({ name }, index) => {
          ...
          try {
            ...
            for (let j = 0, outputChannelSplitterNodeOutput = 0; j < options.numberOfOutputs; j += 1) {
              for (let k2 = 0; k2 < outputChannelCount[j]; k2 += 1) {
                copyToChannel(outputBuffer, outputs[j], k2, outputChannelSplitterNodeOutput + k2, i);
              }
              outputChannelSplitterNodeOutput += outputChannelCount[j];
            }
          } catch (error) {
          ...
          if (!isActive) {
            for (let j = 0; j < options.numberOfInputs; j += 1) {
              ...
              for (let k2 = 0; k2 < options.channelCount; k2 += 1) {
                inputChannelSplitterNodes[i].disconnect(inputChannelMergerNode, k2, j * options.channelCount + k2);
              }
            }
            if (processorConstructor.parameterDescriptors !== void 0) {
...
const EnvelopeCurves = (() => {
  ...
  for (i = 0; i < curveLen - 1; i++) {
    ...
    rippleCurve[i] = sineWave / 10 + k2 * 0.83;
  }
  ...
  for (i = 0; i < curveLen; i++) {
    k2 = i / (curveLen - 1);
    sineCurve[i] = 0.5 * (1 - Math.cos(Math.PI * k2));
  }
  const bounceCurve = [];
  for (i = 0; i < curveLen; i++) {
    k2 = i / (curveLen - 1);
    ...
    bounceCurve[i] = Math.abs(val * (1 - k2));
  }
...
let Instrument$2 = class Instrument extends ToneAudioNode {
  constructor() {
...
class Monophonic extends Instrument$2 {
  constructor() {
  ...
  static getDefaults() {
    return Object.assign(Instrument$2.getDefaults(), {
      detune: 0,
      onsilence: noOp,
      portamento: 0
    });
...
class PolySynth extends Instrument$2 {
  constructor() {
...
class Sampler extends Instrument$2 {
  constructor() {
...
function getEffectiveHand(note, assignment) {
...
function buildScheduledNotes(song) {
  ...
  }).filter((note) => note !== null).sort((left, right) => {
...
function applyTrackAssignments(song, assignments) {
  return {
    ...song,
...
function setTrackAssignment(song, trackId, assignment) {
  return {
    ...song,
...
class AudioEngine {
  ...
  async noteOn(note, velocity = 0.8) {
    ...
    this.synth?.triggerAttack(noteName, now(), velocity);
  }
  noteOff(note) {
    const noteName = midiToName(note);
  ...
  async playSong(song, startSec, tempoMultiplier) {
    await this.init();
    this.pauseSong();
    Transport.cancel(0);
    Transport.position = 0;
    Transport.bpm.value = song.bpm * tempoMultiplier;
  ...
  setTempo(song, startSec, tempoMultiplier, shouldResume) {
    if (!shouldResume) {
      this.seek();
      return Promise.resolve();
    }
...
const MIN_MIDI$1 = 21;
const MAX_MIDI$1 = 108;
const TOTAL_KEYS = MAX_MIDI$1 - MIN_MIDI$1 + 1;
...
class GameSession {
  ...
  combo = 0;
  sustainDown = false;
  physicalInputNotes = /* @__PURE__ */ new Set();
  activeInputNotes = /* @__PURE__ */ new Set();
  constructor(song, tempoMultiplier = 1) {
    this.song = song;
    this.tempoMultiplier = tempoMultiplier;
    this.resetScheduledNotes();
  }
  loadSong(song, nowMs, keepTime = false) {
  ...
  setTempo(tempoMultiplier, nowMs) {
  ...
  getCurrentTimeSec(nowMs) {
    ...
    const nextTime = this.playbackAnchorSec + elapsedMs / 1e3 * this.tempoMultiplier;
    return Math.min(nextTime, this.song.durationSec);
  }
  ingestMidiEvent(event) {
  ...
  matchNote(midi, eventSongTime) {
    const candidate = this.scheduledNotes.find(
      (note) => note.judgement === "pending" && note.midi === midi && Math.abs(note.startSec - eventSongTime) <= HIT_WINDOW_SEC
    );
    if (!candidate) {
      return;
    }
  ...
  markMissedNotes(currentTimeSec) {
  ...
  buildVisibleNotes(currentTimeSec) {
    ...
    return this.scheduledNotes.filter((note) => {
      ...
      const keyIndex = note.midi - MIN_MIDI$1;
  ...
  buildUpcomingNotes(currentTimeSec) {
    ...
    return [...upcoming.entries()].map(([midi, hand]) => ({ midi, hand })).sort((left, right) => left.midi - right.midi);
  }
}
var Midi = {};
var midiFile = {};
function parseMidi(data) {
  ...
  var tracks = [];
  for (var i = 0; !p2.eof() && i < header.numTracks; i++) {
...
Writer.prototype.writeChunk = function(id2, data) {
  this.writeString(id2);
  ...
  var Header2 = (
    ...
    function() {
      ...
      Header3.prototype.ticksToSeconds = function(ticks) {
        var index = (0, BinarySearch_12.search)(this.tempos, ticks);
        if (index !== -1) {
      ...
      Header3.prototype.ticksToMeasures = function(ticks) {
        var index = (0, BinarySearch_12.search)(this.timeSignatures, ticks);
        if (index !== -1) {
      ...
      Header3.prototype.secondsToTicks = function(seconds) {
        var index = (0, BinarySearch_12.search)(this.tempos, seconds, "time");
        if (index !== -1) {
          var tempo = this.tempos[index];
          var tempoTime = tempo.time;
...
var Track = (
  ...
  function() {
    ...
    Track2.prototype.addCC = function(props) {
      var header = privateHeaderMap.get(this);
      var cc2 = new ControlChange_1.ControlChange({
        controllerType: props.number
      }, header);
      delete props.number;
      Object.assign(cc2, props);
      if (!Array.isArray(this.controlChanges[cc2.number])) {
        this.controlChanges[cc2.number] = [];
      }
      (0, BinarySearch_1.insert)(this.controlChanges[cc2.number], cc2, "ticks");
      return this;
    };
    Track2.prototype.addPitchBend = function(props) {
      var header = privateHeaderMap.get(this);
      var pb2 = new PitchBend_1.PitchBend({}, header);
      Object.assign(pb2, props);
      (0, BinarySearch_1.insert)(this.pitchBends, pb2, "ticks");
      return this;
    };
    ...
    Track2.prototype.fromJSON = function(json) {
      ...
      for (var number in json.controlChanges) {
        if (json.controlChanges[number]) {
          json.controlChanges[number].forEach(function(cc2) {
            _this.addCC({
              number: cc2.number,
              ticks: cc2.ticks,
              value: cc2.value
    ...
    Track2.prototype.toJSON = function() {
      ...
      var json = {
        ...
        controlChanges,
        pitchBends: this.pitchBends.map(function(pb2) {
          return pb2.toJSON();
        }),
...
function encodeControlChange(cc2, channel) {
  return {
    absoluteTime: cc2.ticks,
    channel,
    controllerType: cc2.number,
    deltaTime: 0,
    type: "controller",
    value: Math.floor(cc2.value * 127)
  };
}
function encodeControlChanges(track) {
  ...
  for (var i = 0; i < 127; i++) {
    if (track.controlChanges.hasOwnProperty(i)) {
      track.controlChanges[i].forEach(function(cc2) {
        controlChanges.push(encodeControlChange(cc2, track.channel));
      });
    }
  }
  return controlChanges;
}
function encodePitchBend(pb2, channel) {
  return {
    absoluteTime: pb2.ticks,
    channel,
    deltaTime: 0,
    type: "pitchBend",
    value: pb2.value
...
function encodePitchBends(track) {
  var pitchBends = [];
  track.pitchBends.forEach(function(pb2) {
    pitchBends.push(encodePitchBend(pb2, track.channel));
  });
  return pitchBends;
}
...
function velocityFromMidi(value) {
  return Math.max(0, Math.min(1, value / 127));
}
class MidiInputService {
  access = null;
  devices = [];
  midiListeners = /* @__PURE__ */ new Set();
  deviceListeners = /* @__PURE__ */ new Set();
  async init() {
  ...
  refreshDevices() {
    ...
    this.devices = Array.from(this.access.inputs.values()).map((input) => ({
      ...
      manufacturer: input.manufacturer ?? void 0
    }));
  }
  bindInputs() {
    if (!this.access) {
  ...
  normalizeMessage(sourceId, event) {
    if (!event.data) {
      return null;
    }
...
function ControlBar(props) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "control-bar panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "control-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Phase 1" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "Piano Hero" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "song-title", children: props.songTitle })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "transport-buttons", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "secondary-button", onClick: props.onImport, children: "Import MIDI" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "primary-button", onClick: props.onPlayPause, disabled: !props.canPlay, children: props.isPlaying ? "Pause" : "Play" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "secondary-button", onClick: props.onRestart, disabled: !props.canPlay, children: "Restart" })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "control-grid", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "tempo-card", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Tempo" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
          Math.round(props.tempo * 100),
          "%"
        ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "progress-card", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Song Position" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
...
function FallingNotesCanvas({ snapshot, onFileDrop }) {
  const canvasRef = reactExports.useRef(null);
  const containerRef = reactExports.useRef(null);
  const [isDropping, setIsDropping] = reactExports.useState(false);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "canvas-labels", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Falling Notes" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Play at the hit line" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "panel-copy", children: "Drag and drop a MIDI file here or use the import button." })
        ] }),
...
function drawGrid(context, width, height, hitLineRatio) {
  context.fillStyle = "#fdf9f1";
  context.fillRect(0, 0, width, height);
  context.strokeStyle = "rgba(35, 33, 28, 0.08)";
  context.lineWidth = 1;
  for (let index = 0; index <= 11; index += 1) {
    const x2 = width * index / 11;
    context.beginPath();
    context.moveTo(x2, 0);
  ...
  for (let index = 1; index <= 4; index += 1) {
    const y2 = height * hitLineRatio * index / 4;
    context.beginPath();
    context.moveTo(0, y2);
...
function drawNotes(context, width, height, notes) {
  for (const note of notes) {
    const x2 = note.xRatio * width;
    const noteWidth = Math.max(width * note.widthRatio * 0.92, 12);
    const y2 = note.topRatio * height;
    const noteHeight = Math.max(note.heightRatio * height, 14);
    context.fillStyle = noteFill(note);
    context.fillRect(x2 + 2, y2, noteWidth - 4, noteHeight);
    context.fillStyle = "rgba(255, 250, 244, 0.96)";
    context.font = '12px "Alegreya Sans", "Trebuchet MS", sans-serif';
    context.textAlign = "center";
    context.fillText(note.label, x2 + noteWidth / 2, y2 + Math.min(18, noteHeight - 4));
  }
}
function drawHitLine(context, width, height, hitLineRatio) {
  const y2 = height * hitLineRatio;
...
const MIN_MIDI = 21;
const MAX_MIDI = 108;
const BLACK_CLASSES = /* @__PURE__ */ new Set([1, 3, 6, 8, 10]);
...
function buildKeyLayout() {
  const whiteIndexMap = /* @__PURE__ */ new Map();
  ...
  for (let midi = MIN_MIDI; midi <= MAX_MIDI; midi += 1) {
    ...
    if (!isBlack) {
      const left2 = (whiteIndexMap.get(midi) ?? 0) * WHITE_KEY_WIDTH;
      layout.push({ midi, note, isBlack, left: left2 });
      continue;
    }
...
function whiteBeforeMidi(targetMidi) {
  let count = 0;
...
function upcomingMap(upcomingNotes) {
  return new Map(upcomingNotes.map((note) => [note.midi, note.hand]));
}
function PianoKeyboard({ activeNotes, upcomingNotes }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "keyboard-shell panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-heading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Keyboard" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Live + upcoming notes" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "panel-copy", children: "Blue cues the left hand. Orange cues the right." })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "keyboard-stage", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "white-keys", children: whiteKeys.map((key) => {
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            ...
            title: key.note,
            children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: key.note.startsWith("C") ? key.note : "" })
          },
          key.midi
        );
      }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "black-keys", children: blackKeys.map((key) => {
        const cue = upcoming.get(key.midi);
        const isActive = activeSet.has(key.midi);
        return /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
...
const ASSIGNMENT_OPTIONS = ["left", "right", "both", "ignore"];
...
function TrackAssignmentPanel({ tracks, onAssignmentChange }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "track-panel panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "panel-heading", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "eyebrow", children: "Track Map" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: "Hand Assignment" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "panel-copy", children: "Defaults are pitch-based. Adjust each track before playback." })
    ] }),
    tracks.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "empty-state", children: "Import a MIDI file to assign left and right hand tracks." }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "track-list", children: tracks.map((track) => /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "track-row", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: track.name }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          "Default: ",
          formatAssignment(track.defaultAssignment)
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(
        "select",
        {
          ...
          onChange: (event) => onAssignmentChange(track.id, event.target.value),
          children: ASSIGNMENT_OPTIONS.map((assignment) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: assignment, children: formatAssignment(assignment) }, assignment))
        }
      )
    ] }, track.id)) })
  ] });
...
async function createSongId(bytes) {
...
function App() {
  const midiServiceRef = reactExports.useRef(null);
  const audioEngineRef = reactExports.useRef(new AudioEngine());
  const gameSessionRef = reactExports.useRef(null);
  const rafRef = reactExports.useRef(null);
  const [song, setSong] = reactExports.useState(null);
  const [snapshot, setSnapshot] = reactExports.useState(EMPTY_SNAPSHOT);
  const [tempo, setTempo] = reactExports.useState(1);
  const [devices, setDevices] = reactExports.useState([]);
  const [midiError, setMidiError] = reactExports.useState(null);
  const [statusMessage, setStatusMessage] = reactExports.useState("Import a MIDI file and connect a keyboard to begin.");
  const [sourcePath, setSourcePath] = reactExports.useState();
  reactExports.useEffect(() => {
  ...
  const ensureAudioReady = async () => {
    try {
      ...
      setStatusMessage(`Audio failed to initialize: ${error.message}`);
    }
  };
  const loadSong = async (arrayBuffer, title, sourcePath2) => {
    ...
    const hydratedSong = storedMetadata ? applyTrackAssignments(parsedSong, storedMetadata.trackAssignments) : parsedSong;
  ...
  const handlePlayPause = async () => {
    ...
    await audioEngineRef.current.playSong(currentSong, game.getCurrentTimeSec(now2), tempo);
    game.play(now2);
  ...
  const handleSeek = async (progress2) => {
    ...
    const now2 = performance.now();
    const targetSec = progress2 * currentSong.durationSec;
    const shouldResume = game.isTransportPlaying();
    game.seek(targetSec, now2);
    if (shouldResume) {
  ...
  const handleAssignmentChange = async (trackId, assignment) => {
    if (!song) {
      return;
    }
    ...
    gameSessionRef.current = nextGame;
    setSnapshot(nextGame.getSnapshot(now2));
    if (window.appBridge) {
      const metadata = {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "app-shell", onPointerDownCapture: ensureAudioReady, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx(
      ControlBar,
      {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "status-strip", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "status-card", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Combo" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: snapshot.combo })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "status-card", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "MIDI Devices" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: devices.length > 0 ? devices.map((device) => device.name).join(", ") : "No devices detected" })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "status-card wide", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Status" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: midiError ? `MIDI unavailable: ${midiError}` : statusMessage })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "workspace-grid", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(FallingNotesCanvas, { snapshot, onFileDrop: (file) => void handleDroppedFile(file) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(TrackAssignmentPanel, { tracks: song?.tracks ?? [], onAssignmentChange: (trackId, assignment) => void handleAssignmentChange(trackId, assignment) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(PianoKeyboard, { activeNotes: snapshot.activeInputNotes, upcomingNotes: snapshot.upcomingNotes })
  ] });
}
