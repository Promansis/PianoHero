...
const ACCESS_COOKIE_NAME = 'lumakeys_web_access';
...
export function createApiAccessGate(secret: string | undefined): MiddlewareHandler {
  ...
  return async (c, next) => {
    ...
    const suppliedSecret =
      c.req.header('x-lumakeys-access-token') ??
    ...
    if (c.req.header('x-lumakeys-access-token') === expectedSecret || new URL(c.req.url).searchParams.get('access_token') === expectedSecret) {
